...
function checkForSession() {
  fetchSession().then(populateTodos).catch(() => setLoggedIn(false));
}
...
function addAbilityToToggleComplete() {
  ...
  listEl.addEventListener('click', e => {
    ...
    const id = e.target.dataset.id;
    fetchUpdateTodo(id, { done: !state.todos[id].done }).then(todo => {
      state.todos[id] = todo;
      render({ state });
      renderStatus('');
    }).catch(err => {
